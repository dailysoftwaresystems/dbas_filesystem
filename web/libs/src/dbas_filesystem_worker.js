...
async function handleMessage(method, args) {
    switch (method) {
        ...
        case 'initialize': {
            ...
            // Clean up orphaned streams from a previous crash/restart.
            if (activeWritables.size > 0) {
                for (const [streamId, writable] of activeWritables) {
                    try { await writable.abort(); } catch (e) {
                        console.warn('Failed to abort orphaned stream ' + streamId + ':', e);
                    }
                }
        ...
        case 'renameDirectory': {
            ...
            try {
                ...
                try { await handleMessage('deleteDirectory', { path: args.newPath, recursive: true }); } catch (cleanupErr) {
                    console.warn('renameDirectory cleanup failed for ' + args.newPath + ':', cleanupErr);
                }
                throw e;
            }
            return true;
        ...
        case 'moveDirectory': {
            ...
            // Check destination doesn't already exist (match native behaviour).
            let destExists = true;
            try { await navigateToDir(args.destPath); } catch (e) {
                if (e.name === 'NotFoundError') destExists = false;
                else throw e;
            }
            if (destExists) {
                throw fsError('DIRECTORY_NOT_EMPTY', args.destPath);
            }
            try {
                await copyDirectoryRecursive(srcDir, args.destPath, true);
                await handleMessage('deleteDirectory', { path: args.sourcePath, recursive: true });
            } catch (e) {
                try { await handleMessage('deleteDirectory', { path: args.destPath, recursive: true }); } catch (cleanupErr) {
                    console.warn('moveDirectory cleanup failed for ' + args.destPath + ':', cleanupErr);
                }
                throw e;
            }
            return true;
